...
const socket = io('https://thevoidbackend.onrender.com');
...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchUsernames = async () => {
      try {
        const response = await fetch('https://randomuser.me/api/?results=5&inc=login');
  ...
  return (
    <div style={styles.chatWrapper}>
      ...
      <div style={styles.chatBox}>
        {chatLog.map((msg, i) => (
          <div key={i} style={msg.user === username ? styles.myMsg : styles.theirMsg}>
...
const styles = {
  setupScreen: { height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: '#000', color: '#fff', fontFamily: 'monospace' },
  title: { fontSize: '3rem', letterSpacing: '10px', marginBottom: '40px' },
  nameGrid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '30px' },
  nameBtn: { padding: '10px', backgroundColor: 'transparent', cursor: 'pointer', fontSize: '14px' },
